        ...
        for i in range(0, self.x):
            for j in range(0, self.y):
                ...
                if(revealedBoxes[i][j] == True) and (self.checkedBoxes[i][j] == 0):
                    if(mineField[i][j] in self.validNumberedBoxes):
                        ...
                        for xi in range(-1, 2):
                            for yi in range(-1, 2):
                                if(probabilities[i+xi][j+yi] < lowestProbability) and (revealedBoxes[i+xi][j+yi] == False) and ((i+xi >= 0) and (j+yi >= 0)) and ((i+xi <= self.x - 1) and (j+yi <= self.y)):
                                    lowestX = i+xi
                                    lowestY = j+yi
    ...
    def calculateProbability(self, x, y, revealedBoxes):
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                ...
                if ((x + i >= 0) and (y + j >= 0)) and ((x + i <= self.x - 1) and (y + j <= self.y - 1)):
                    if revealedBoxes[x + i][y + i] == False:
                        unrevealedBoxes+=1
    ...
    def findNextNumberedBox(self, x, y, revealedBoxes, mineField):
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                ...
                if (not(x + i < 0) or not(x + i > x)) and x != -1 and y != 1:
                    pass
                elif (revealedBoxes[x + i][y + j] == True) and (mineField[x + i][y + j] in self.validNumberedBoxes) and (i !=0 and j != 0):
                    nextBlockX = x + i
                    nextBlockY = y + j
